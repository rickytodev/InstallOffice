...
def install():
    ...
    command = f'"{
        r}/setup" /configure "{r}/config{platform.architecture()[0]}s.xml"'
...
class Finish:
    ...
    def __init__(self):
        ...
        publisher = Label(
            window,
            text=f" Publisher - {DATA['publisher']}",
            font=("Arial", 11),
            bg="white",
            image=publisher_img,
            compound="left",
        ...
        button_finish.bind(
            "<ButtonPress-1>", lambda event: (window.destroy(), remove_installation())
        )
...
class Main:
    ...
    def __init__(self):
        ...
        button_install.bind(
            "<ButtonPress-1>", lambda event: (window.destroy(), install())
        )
